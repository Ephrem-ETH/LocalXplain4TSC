...
def parse_arguments():
    ...
    parser.add_argument("--model", required=False, default=FCN, help="Model name")
...
def run_single_iteration(base_dir, run, X, y, model_name, class_names,replacement_method, num_samples):
    ...
    try:
        ...
        important_features_lr, prediction_score, local_pred = globxplain_instance.ts_local_explanation(randomly_picked_instance, learn_new, num_perturbations=num_samples, class_names=class_names, replacement_method=replacement_method)
...
if __name__ == "__main__":
    ...
    base_dir = f'results/simulation/{dataset_name}/{model}-{r_method}--{cur_time}' if 'cuda' in str(device) else f'results/{dataset_name}--{cur_time}'
    ...
    try:
        ...
        result_dict = monte_carlo_cross_val_parallel(base_dir, FCN, num_runs, class_labels, r_method)

        

       # Define the table result
        fcn_result = [
        ...
        print(tabulate(fcn_result, headers=table_headers, tablefmt="grid"))
